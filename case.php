...
$sqs="SELECT accusedID FROM cases ORDER BY accusedID DESC LIMIT 1";
 ...
 echo '<script type="text/javascript">alert("Failed!! entered not allowed accuserID or accusedID.");window.location=\'1case_register.php\';</script>';	 
 }
 else if($date>date('Y/m/d')){
	 echo '<script type="text/javascript">alert("Failed!! entered invalid date.");window.location=\'1case_register.php\';</script>'; 
 }
 else{
	 $sql="INSERT INTO
...
if($sql){
 echo '<script type="text/javascript">alert(" Register successfully!!");window.location=\'1case_register.php\';</script>';
}
if(!$retval ) { 
die('Could not enter data: '. mysqli_error($db));
require_once '1case_register.php'; 
 }
 }
	}                       
 ?>				
